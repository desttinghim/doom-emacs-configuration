...
(defconst jasmin-version "2.4")

(defconst jasmin-author "Neil W. Van Dyke <neil@neilvandyke.org>")

(defconst jasmin-maintainer-address "neil@neilvandyke.org")
...
(defconst jasmin-directives
  '(
    ...
    ;; Jasmin XT
    (bytecode   . [global (version-number)])
    ;; (debug      . [global (value)])
    (enclosing  . [global ("method" method-name-and-signature )])
    ;; (signature  . [method (value)])
    (inner      . [method (("class" "interface") access-spec class-name ("inner" "outer") class-name)])
    ;; (attribute  . [method (name file-name)])
    ;; (annotation . [method ()])
    ;; (stack      . [method ()])
    ;; (deprecated . [method ()])
    ))
...
(defconst jasmin-instructions
  '(
    ...
    (invokenonvirtual . [(method-spec)
                         "(Renamed to invokespecial) Invoke instance method via compile-time type"])
...
(defconst jasmin-access-specs
  '(abstract final interface native private protected public static
             synchronized transient volatile super))
  ...
  (if (eq token '&opt)
    ...
    (let* ((default-regexp "[^ \t\f\n\^m]+\\>")
      ...
      (cond ((eq token 'access-spec)
            ...
            ((eq token 'version-number) default-data)
            ((eq token 'integer)
